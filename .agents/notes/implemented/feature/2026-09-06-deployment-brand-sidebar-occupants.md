...
**Replacing `apps/web/public/favicon.svg` for the tab icon.** Zero runtime code and it would also reach the web manifest, but it is a host asset shared by every build profile, so an `official` build would carry the Kasyun icon too. The runtime effect keeps the icon under the same gate as the occupants; the manifest stays a recorded limitation.
