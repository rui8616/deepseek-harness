/** Deployment-owned occupants for the generic sidebar brand slots. */
...
import { CustomBrandMark, CustomBrandName } from './Brand.tsx'
 ...
 * build gets this deployment's brand. The conversation hero stays on its
 * declaring package's animated fish fallback.
 * @param ctx - Client root context.
 */
export function apply(ctx: ClientContext): void {
  if (process.env.DSH_CLIENT_BUILD_PROFILE === 'official') return
  ctx.effect(() => ctx.locale.register(NS, { zh, en, ja }), 'ui-brand-custom: dictionaries')
  ctx.slots.inject('sidebar.brand.mark', () =>
    ctx.slots.inject('sidebar.brand.name', function* () {
      yield ctx.slots.register({ name: 'sidebar.brand.mark' }, CustomBrandMark)
      yield ctx.slots.register({ name: 'sidebar.brand.name', locale: NS }, CustomBrandName)
    }))
}
