...
describe('isTrustedApiRequest', () => {
  it('holds markerless requests to the same Host fence — a plain-HTTP browser read carries no markers', () => {
    ...
    expect(isTrustedApiRequest(request({ host: '127.0.0.1:4500' }), [])).toBe(true)
    expect(isTrustedApiRequest(request({ host: '192.168.1.5:4500' }), ['192.168.1.5'])).toBe(true)
    expect(isTrustedApiRequest(request({ host: '192.168.1.5:4500' }), [])).toBe(false)
  ...
  it('accepts loopback Hosts in every spelling, with and without ports, for browser requests', () => {
    for (const host of ['localhost', 'localhost:4500', '127.0.0.1', '127.0.0.1:4500', '127.8.9.10:80', '[::1]', '[::1]:4500', 'LOCALHOST:4500']) {
  ...
  it('refuses a rebound Host: the attacker domain names the socket it did not expect', () => {
    expect(isTrustedApiRequest(request({
      host: 'evil.example:4500',
      origin: 'http://evil.example:4500',
      'sec-fetch-site': 'same-origin',
    }), [])).toBe(false)
  })

  it('accepts a declared public authority: exact on host:port entries, any port on port-less entries', () => {
    const headers = { host: 'harness.internal:4500', origin: 'http://harness.internal:4500' }
    expect(isTrustedApiRequest(request(headers), ['harness.internal:4500'])).toBe(true)
  ...
  it('matches Host, Origin, and trusted entries through WHATWG normalization (case, default port)', () => {
    expect(isTrustedApiRequest(request({ host: 'Harness.INTERNAL:4500', origin: 'http://harness.internal:4500' }), ['harness.internal:4500'])).toBe(true)
  ...
  it('refuses cross-origin browser markers even on a loopback Host', () => {
    ...
    expect(isTrustedApiRequest(request({ host: '127.0.0.1:4500', origin: 'http://evil.example' }), [])).toBe(false)
    // Explicit cross-site label → refused regardless of Origin.
    expect(isTrustedApiRequest(request({ host: '127.0.0.1:4500', 'sec-fetch-site': 'cross-site' }), [])).toBe(false)
    // Opaque origin (sandboxed iframe, file: page) parses to no authority.
    expect(isTrustedApiRequest(request({ host: '127.0.0.1:4500', origin: 'null' }), [])).toBe(false)
  })
  ...
  it('accepts a same-origin browser request, with or without an Origin header', () => {
    ...
    expect(isTrustedApiRequest(request({ host: 'localhost:4500', 'sec-fetch-site': 'same-origin' }), [])).toBe(true)
  })

  it('reads Fetch Headers while preserving absent browser markers', () => {
    expect(isTrustedApiRequest({ headers: new Headers({ host: '127.0.0.1:4500' }) }, [])).toBe(true)
    expect(isTrustedApiRequest({
      headers: new Headers({ host: '127.0.0.1:4500', origin: 'http://evil.example' }),
    }, [])).toBe(false)
  })

  it('assertTrustedAuthority accepts bare authorities and throws on anything more', () => {
    for (const entry of ['harness.internal', 'harness.internal:4500', 'HARNESS.internal:80', '10.0.0.9', '[::1]:4500']) {
    ...
    for (const entry of ['harness.internal:4500 ', ' harness.internal', 'harness.internal:30\t80']) {
      expect(() => { assertTrustedAuthority(entry) }).toThrow(/not a bare host\[:port\] authority/)
    }
  ...
  it('never lets stray whitespace broaden an exact-port entry to every port', () => {
    ...
    const trusted = ['harness.internal:4500 ']
    expect(isTrustedApiRequest(request({ host: 'harness.internal:9999', origin: 'http://harness.internal:9999' }), trusted)).toBe(false)
    expect(isTrustedApiRequest(request({ host: 'harness.internal:4500', origin: 'http://harness.internal:4500' }), trusted)).toBe(true)
  })
