---
description: "面向用户与插件作者的 web GUI 本地化说明：zh/en/ja 偏好、浏览器派生回退、类型化命名空间词典与框架翻译席位。"
...
用已合并进 `LocaleNamespaceMap` 的命名空间调用 `ctx.locale.register(ns, { zh, en, ja })`；编译器会对照该命名空间的类型化键并集检查每个键，并要求每个内置 locale 齐全。消费方通过 `ctx.locale.bind(ns)` 或框架注入的 `t` 席位翻译。UI 已挂载后再注册的词典无需重新挂载即可生效。
...
带类型的对象形式要求每个内置 locale 都有完整字典；逐 locale 形式允许语言包独立注册每个命名空间。逐键查找会先在请求命名空间中沿生效语言声明的 fallback 链查找，再在 `common` 中重复该链，最后显示键本身。绑定的翻译函数按命名空间保持稳定身份，因此可以挂在 inject 表面上而不破坏 memoization。
...
| [`src/locales/`](src/locales/) | 已发布的 `zh`／`en`／`ja` 词典 |
